...
class TexValidator:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if os.path.exists(images_base_dir):
                for dir_name in os.listdir(images_base_dir):
                    ...
                    if os.path.isdir(dir_path) and session_id and session_id in dir_name:
                        actual_images_dir = dir_path
                        break
                
                # 如果是修订session（以revision_开头），尝试查找原始session的图片
                if not actual_images_dir and session_id and session_id.startswith("revision_"):
                    # 查找最新的非revision目录作为图片源
                    non_revision_dirs = [d for d in os.listdir(images_base_dir) 
                                       if os.path.isdir(os.path.join(images_base_dir, d)) 
                                       and not d.startswith("revision_")]
                    if non_revision_dirs:
                        # 按时间排序，取最新的
                        non_revision_dirs.sort(reverse=True)
                        actual_images_dir = os.path.join(images_base_dir, non_revision_dirs[0])
                        self.logger.info(f"修订模式：复制图片从原始目录 {actual_images_dir}")
    ...
    def _process_image_references(self, tex_file: str, images_dir: str):
        ...
        # 先修复图片引用的参数格式，防止Division by 0错误
        def fix_includegraphics(match):
            image_path = match.group(1)
            # 使用标准的安全参数，防止除零错误
            safe_params = "[width=0.7\\textwidth, height=0.4\\textheight, keepaspectratio]"
            return f"\\includegraphics{safe_params}{{{image_path}}}"
        
        # 修复所有includegraphics命令，统一使用安全参数
        includegraphics_pattern = r'\\includegraphics(?:\[[^\]]*\])?\{([^}]+)\}'
        original_count = len(re.findall(includegraphics_pattern, tex_content))
        tex_content = re.sub(includegraphics_pattern, fix_includegraphics, tex_content)
        self.logger.info(f"已修复 {original_count} 个图片引用的参数格式")
        ...
        # 从TEX文件中的图片路径推断正确的图片目录
        actual_images_dir = None
        
        if matches:
            # 从第一个图片路径中提取图片目录
            first_img_path = matches[0]
            # 例如: output/images/1757128177/_page_0_Picture_0.jpeg -> output/images/1757128177
            img_dir_match = re.match(r'(output/images/\d+)', first_img_path)
            if img_dir_match:
                inferred_images_dir = img_dir_match.group(1)
                if os.path.exists(inferred_images_dir) and os.path.isdir(inferred_images_dir):
                    actual_images_dir = inferred_images_dir
                    self.logger.info(f"从TEX路径推断图片目录: {actual_images_dir}")
        
        # 如果推断失败，使用原有的查找逻辑作为备用
