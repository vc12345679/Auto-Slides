...
2.  **Field Importance & Context (1-2 slides)**:
...
[
  {{
    ...
    "figure_reference": {{
      "id": "figure_ID",
      "description": "A figure that represents the broader context or importance of the field"
    }},
  ...
  {{
    ...
    "figure_reference": {{
        "id": "figure_overview",
        "description": "Framework overview showing the automated design process"
    }},
    "includes_table": false,
    "table_reference": null,
  ...
  {{
    ...
    "includes_figure": true,
    "figure_reference": {{
        "id": "figure_methodology",
        "description": "Graph representation and hierarchical search space illustration"
    }},
    "includes_table": false,
    "table_reference": null,
    "presenter_notes": "Explain the technical foundation of the approach."
  }},
  {{
    "slide_number": 5,
...
**Figure Integration & Table Requirements:**
- Figures should be prioritized for integration into slides most relevant to their content (e.g., methodology, results, analysis), avoiding standalone slides unless specifically needed for detailed explanation.
- Each figure's `figure_reference` should be closely related to the slide content.
- The Results section must have at least one slide that combines key tables (`includes_table: true`) with bullet-point analysis (`content` field), i.e., tables and analysis points should be displayed on the same slide.
...
- **UNIQUE FIGURE ASSIGNMENT**: Each figure should appear ONLY ONCE in the entire presentation
  * Track used figures across all slides to prevent duplicates
  * Each figure should be assigned to the slide where it provides maximum value
  * **MANDATORY**: Before assigning a figure, check if it has been used in previous slides
- **Content-Figure Alignment**: Analyze slide content and match it with the most relevant figure based on:
  * **Topic Match**: Figure should illustrate the main concept discussed in the slide
  * **Level Match**: Overview figures for overview slides, detailed figures for detailed slides  
  * **Purpose Match**: Method figures for method slides, result figures for result slides
- **Figure Priority Ranking**: When multiple slides could use the same figure, prioritize assignment based on:
  1. **Core methodology slides** (highest priority)
  2. **Key results slides** (high priority)  
  3. **Problem illustration slides** (medium priority)
  4. **Background/context slides** (lowest priority)
- **Avoid Mismatched Combinations**:
  * Do NOT use overview figures for detailed technical explanations
  * Do NOT use result graphs for method description slides
  * Do NOT use architecture diagrams for background/motivation slides
- **Smart Figure Selection**:
  * If slide discusses "3 levels" or "hierarchical structure", find figures showing layered/hierarchical content
  * If slide covers "mechanism" or "process", find figures showing workflows or pipelines
  * If slide presents "architecture" or "network design", find figures showing model architectures
  * If slide shows "results" or "comparison", find figures with graphs, charts, or comparative visualizations
- **Figure Quality Requirements**:
  * Each figure MUST have clear connection to the slide's main message
  * Figure caption should reflect the slide content focus
  * If no truly relevant figure exists for a slide, set includes_figure to false rather than force a mismatch
  * **NO REPETITION**: If a figure has been used, find alternative ways to illustrate content or use text-only slides
