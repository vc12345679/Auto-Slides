...
你的任务是：
1. **理解意图**: 完全理解修改指令的意图。
2. **参考上下文**: 如果修改需要从文档的其他部分获取信息（如标题页的机构信息、其他页面的格式等），请在完整文档中查找相关信息。
3. **精确修改**: 只修改需要修改的部分，保持其余代码（包括页码标记）不变。
4. **代码质量**: 确保生成的代码语法正确，格式良好。
输出格式为JSON，包含`modified_code`字段。

例如，如果指令是"补充完整的机构信息"，你应该在完整文档中查找\institute{}或相关内容，并用真实的机构信息替换占位符。
"""
        prompt = f"这是完整的LaTeX文档内容（供参考）:\n```latex\n{full_document_context}\n```\n\n这是需要修改的代码片段:\n```latex\n{original_snippet}\n```\n\n请根据以下指令修改它:\n{instruction}"
                    ...
                    for step in plan:
                        ...
                        if step['action'] == 'locate':
                            ...
                            if not current_snippet:
                                print("❌ 修改失败，前一步定位未成功。")
                                break
                            modified_snippet = self.generate_modified_code(current_snippet, step['description'], self.document_content)
